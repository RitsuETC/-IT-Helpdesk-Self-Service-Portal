...
router.delete('/assets/:id', adminOnly, async (req, res) => {
  try {
    const result = await db.query('DELETE FROM asset WHERE id_asset = $1', [req.params.id]);
...
router.post('/spareparts', adminOnly, async (req, res) => {
  try { const values = sparepartFields.map((field) => req.body[field] ?? null); const { rows } = await db.query(`INSERT INTO sparepart (${sparepartFields.join(', ')}) VALUES (${values.map((_, i) => `$${i + 1}`).join(', ')}) RETURNING *`, values); res.status(201).json({ data: rows[0] }); }
...
router.delete('/spareparts/:id', adminOnly, async (req, res) => {
  try { const result = await db.query('DELETE FROM sparepart WHERE id = $1', [req.params.id]); if (!result.rowCount) return res.status(404).json({ message: 'Sparepart tidak ditemukan' }); res.json({ message: 'Sparepart berhasil dihapus' }); }
...
router.post('/movements', workRoles, async (req, res) => {
  ...
  try { const { id_asset, id_user, from_location, to_location, movement_type, movement_date, condition, notes, id_pic } = req.body; if (!positiveInt(id_asset) || !movement_type) return res.status(400).json({ message: 'Aset dan jenis pergerakan wajib diisi' }); await client.query('BEGIN'); const result = await client.query('INSERT INTO asset_movement (id_asset, id_user, from_location, to_location, movement_type, movement_date, condition, notes, id_pic) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *', [id_asset, id_user || null, from_location || null, to_location || null, movement_type, dateValue(movement_date), condition || null, notes || null, id_pic || req.user.id]); await client.query('UPDATE asset SET id_user = COALESCE($1, id_user), id_ruangan = COALESCE($2, id_ruangan), condition = COALESCE($3, condition), updated_at = NOW() WHERE id_asset = $4', [id_user || null, to_location || null, condition || null, id_asset]); await client.query('COMMIT'); res.status(201).json({ data: result.rows[0] }); } catch (error) { await client.query('ROLLBACK'); res.status(500).json({ message: 'Gagal mencatat pergerakan aset', error: error.message }); } finally { client.release(); }
});
...
router.post('/transactions', workRoles, async (req, res) => {
  const client = await db.connect();
  try { const { id_sparepart, transaction_type, quantity, transaction_date, id_tiket, notes, id_pic } = req.body; if (!positiveInt(id_sparepart) || !['MASUK', 'KELUAR'].includes(transaction_type) || !positiveInt(quantity)) return res.status(400).json({ message: 'Sparepart, jenis transaksi, dan jumlah wajib valid' }); await client.query('BEGIN'); const stock = await client.query('SELECT stock FROM sparepart WHERE id = $1 FOR UPDATE', [id_sparepart]); if (!stock.rows.length) return res.status(404).json({ message: 'Sparepart tidak ditemukan' }); const nextStock = Number(stock.rows[0].stock) + (transaction_type === 'MASUK' ? Number(quantity) : -Number(quantity)); if (nextStock < 0) return res.status(400).json({ message: 'Stok tidak mencukupi' }); const result = await client.query('INSERT INTO sparepart_transaction (id_sparepart, transaction_type, quantity, transaction_date, id_tiket, notes, id_pic) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *', [id_sparepart, transaction_type, quantity, dateValue(transaction_date), id_tiket || null, notes || null, id_pic || req.user.id]); await client.query('UPDATE sparepart SET stock = $1, updated_at = NOW() WHERE id = $2', [nextStock, id_sparepart]); await client.query('COMMIT'); res.status(201).json({ data: result.rows[0] }); } catch (error) { await client.query('ROLLBACK'); res.status(500).json({ message: 'Gagal mencatat transaksi sparepart', error: error.message }); } finally { client.release(); }
});
...
router.delete('/procurement/:id', adminOnly, async (req, res) => { try { const result = await db.query('DELETE FROM procurement WHERE id = $1', [req.params.id]); if (!result.rowCount) return res.status(404).json({ message: 'Pengadaan tidak ditemukan' }); res.json({ message: 'Pengadaan berhasil dihapus' }); } catch (error) { res.status(500).json({ message: 'Gagal menghapus pengadaan', error: error.message }); } });
