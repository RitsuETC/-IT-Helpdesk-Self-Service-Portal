...
function Dashboard({ token, user, onTroubleshooting, onTickets, onKnowledge }) {
  ...
  return <section className="dashboard">
    ...
    <section className="ticket-panel" aria-labelledby="ticket-title"><div className="ticket-panel-title"><h3 id="ticket-title">{isStaff ? 'Seluruh Tiket' : 'Tiket Saya'}</h3>{tickets.length > 1 && <div className="ticket-slider-controls"><button onClick={() => changeTicket(-1)} aria-label="Tiket sebelumnya">‹</button><span>{ticketIndex + 1}/{tickets.length}</span><button onClick={() => changeTicket(1)} aria-label="Tiket berikutnya">›</button></div>}</div>
      <div className={`dashboard-ticket ${isStaff ? 'staff-ticket' : ''}`}>
        ...
        {currentTicket ? <div className="ticket-slide" key={currentTicket.id}><span>HD-{currentTicket.id}</span>{isStaff && <span>{currentTicket.pelapor_nama || '-'}</span>}<span>{currentTicket.nama_kategori || '-'}</span><span>{currentTicket.status || '-'}</span></div> : <p>Belum ada tiket.</p>}
      </div>
    </section>
  </section>
}
