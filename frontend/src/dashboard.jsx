...
function Dashboard({ token, user, onTroubleshooting, onTickets, onKnowledge }) {
  ...
  return <section className="dashboard">
    ...
    <section className="ticket-panel" aria-labelledby="ticket-title"><div className="ticket-panel-title"><h3 id="ticket-title">{isStaff ? 'Seluruh Tiket' : 'Tiket Saya'}</h3>{(isStaff ? activeTickets.length : tickets.length) > 1 && <div className="ticket-slider-controls"><button onClick={() => changeTicket(-1)} aria-label="Tiket sebelumnya">‹</button><span>{ticketIndex + 1}/{isStaff ? activeTickets.length : tickets.length}</span><button onClick={() => changeTicket(1)} aria-label="Tiket berikutnya">›</button></div>}</div>
      <div className={`dashboard-ticket ${isStaff ? 'staff-ticket' : ''}`}>
        ...
        { (isStaff ? activeTickets.length > 0 : tickets.length > 0) ? (() => {
            const list = isStaff ? activeTickets : tickets
