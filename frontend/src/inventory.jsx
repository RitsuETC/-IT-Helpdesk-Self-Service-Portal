...
export default function Inventory({ token, user, onBack, onError }) {
  ...
  const submit = async (event, path, method, body, reset) => {
    ...
    try { 
        ...
        if(path.startsWith('/inventory/spareparts')) setShowPartModal(false);
    } catch (error) { onError(error.message) }
  }
  ...
  const handleEditMaintenance = (item) => { setMaintenance({ ...blankMaintenance, ...item, start_date: toDateTimeLocal(item.start_date), end_date: toDateTimeLocal(item.end_date) }); setMaintenanceEditingId(item.id); }
  ...
  return <section className="inventory-page">
    ...
    {tab === 'spareparts' && (
      <section className="inventory-section">
        ...
        {/* Filter Sparepart yang mirip dengan filter Asset */}
    ...
    {/* ===================== TAB: PERGERAKAN ASET ===================== */}
    {tab === 'movements' && (
      <WorkSection title="Pergerakan Inventaris" items={data.movements} itemsPerPage={ITEMS_PER_PAGE} columns={['item_name', 'movement_type', 'quantity', 'from_room', 'to_room', 'movement_date']} labels={['Item', 'Jenis', 'Jumlah', 'Asal', 'Tujuan', 'Tanggal']}>
        <form className="inventory-form" onSubmit={(event) => submit(event, '/inventory/movements', 'POST', { ...movement, id_asset: movement.id_asset || null, id_sparepart: movement.id_sparepart || null, asset_quantity: Number(movement.asset_quantity || 0), sparepart_quantity: Number(movement.sparepart_quantity || 0), id_user: movement.id_user || null, from_location: movement.from_location || null, to_location: movement.to_location || null, movement_date: movement.movement_date || new Date().toISOString().slice(0, 16) }, () => setMovement(blankMovement))}>
          ...
          {movement.id_asset && <Field label="Jumlah aset *"><Input name="asset_quantity" value={movement.asset_quantity} onChange={update(setMovement)} type="number" required /></Field>}
          ...
          {movement.id_sparepart && <Field label="Jumlah sparepart *"><Input name="sparepart_quantity" value={movement.sparepart_quantity} onChange={update(setMovement)} type="number" required /></Field>}
          ...
          <div style={{ gridColumn: '1 / -1' }}><FormActions label="Catat pergerakan" /></div>
        </form>
      </WorkSection>
    )}
    ...
    {tab === 'transactions' && (
      <WorkSection title="Transaksi Sparepart" items={data.transactions} itemsPerPage={ITEMS_PER_PAGE} columns={['sparepart_name', 'transaction_type', 'quantity', 'transaction_date', 'ticket_title', 'pic_name']} labels={['Sparepart', 'Jenis', 'Jumlah', 'Tanggal', 'Referensi tiket', 'PIC']}>
        <form className="inventory-form" onSubmit={(event) => submit(event, '/inventory/transactions', 'POST', { ...transaction, id_sparepart: Number(transaction.id_sparepart), quantity: Number(transaction.quantity), id_tiket: transaction.id_tiket || null, transaction_date: transaction.transaction_date || new Date().toISOString().slice(0, 16) }, () => setTransaction(blankTransaction))}>
          ...
          <Field label="Jumlah *"><Input name="quantity" value={transaction.quantity} onChange={update(setTransaction)} type="number" required /></Field>
          ...
          <div style={{ gridColumn: '1 / -1' }}><FormActions label="Catat transaksi" /></div>
        </form>
      </WorkSection>
    )}
    ...
    {tab === 'maintenance' && (
      <WorkSection title="Maintenance" items={data.maintenance} itemsPerPage={ITEMS_PER_PAGE} columns={['asset_code', 'maintenance_type', 'start_date', 'end_date', 'status', 'pic_name']} labels={['Aset', 'Jenis', 'Mulai', 'Selesai', 'Status', 'PIC']} renderActions={isAdmin ? (item) => <button type="button" className="secondary-button" onClick={() => handleEditMaintenance(item)}>Edit</button> : null}>
        <form className="inventory-form" onSubmit={(event) => submit(event, maintenanceEditingId ? `/inventory/maintenance/${maintenanceEditingId}` : '/inventory/maintenance', maintenanceEditingId ? 'PUT' : 'POST', { ...maintenance, id_asset: Number(maintenance.id_asset), end_date: maintenance.end_date || null, cost: Number(maintenance.cost || 0) }, () => { setMaintenance(blankMaintenance); setMaintenanceEditingId(null) })}>
          ...
          <Field label="Biaya"><Input name="cost" value={maintenance.cost} onChange={update(setMaintenance)} type="number" /></Field>
    ...
    {/* ===================== TAB: PENGADAAN ===================== */}
    {tab === 'procurement' && (
      <WorkSection title="Pengadaan" items={data.procurement} itemsPerPage={ITEMS_PER_PAGE} columns={['po_number', 'request_date', 'supplier', 'status', 'total_cost']} labels={['Nomor PO', 'Pengajuan', 'Supplier', 'Status', 'Total biaya']}>
        <form className="inventory-form" onSubmit={(event) => submit(event, '/inventory/procurement', 'POST', { ...procurement, details: procurement.details.filter((item) => item.item_name.trim()) }, () => setProcurement(blankProcurement))}>
          ...
          <Field label="Jumlah"><Input type="number" value={procurement.details[0].quantity} onChange={(event) => setProcurement((previous) => ({ ...previous, details: [{ ...previous.details[0], quantity: event.target.value }] }))} /></Field>
          <Field label="Harga satuan"><Input type="number" value={procurement.details[0].unit_price} onChange={(event) => setProcurement((previous) => ({ ...previous, details: [{ ...previous.details[0], unit_price: event.target.value }] }))} /></Field>
          ...
          <div style={{ gridColumn: '1 / -1' }}><FormActions label="Simpan pengadaan" /></div>
        </form>
      </WorkSection>
    )}

    {/* ===================== TAB: MASTER ASSET / PRODUK ===================== */}
    {tab === 'master-products' && (
      <section className="inventory-section">
        <SectionTitle title="Master Produk / SKU" count={masterProductsList.length} search="" />
        {isAdmin && (
          <form className="inventory-form" onSubmit={(event) => { 
              const { sku_code, product_name, id_category, default_price, processor, ram, storage, operating_system, notes } = masterProduct;
              const specifications = Object.fromEntries(Object.entries({ processor, ram, storage, operating_system, notes }).filter(([, value]) => String(value || '').trim()));
              submit(
                event, 
                ...
                { sku_code, product_name, id_category: id_category || null, default_price: default_price || null, specifications }, 
          ...
          <tbody>
            {paginatedMaster.map((item) => (
              <tr key={item.id}>
                ...
                <td>{setup.categories.find((category) => String(category.id) === String(item.id_category))?.name || '-'}</td>
    ...
    {tab === 'master-spareparts' && (
      <section className="inventory-section">
        ...
        {isAdmin && <form className="inventory-form" onSubmit={(event) => { 
        ...
        <div className="modal-backdrop" onClick={() => setShowAssetModal(false)}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: '#fff', borderRadius: '12px', padding: '24px', width: '90%', maxWidth: '800px', maxHeight: '90vh', overflowY: 'auto' }}>
                ...
                <form className="inventory-form" onSubmit={(event) => submit(event, assetEditingId ? `/inventory/assets/${assetEditingId}` : '/inventory/assets', assetEditingId ? 'PUT' : 'POST', { ...asset, id_category: asset.id_category || null, id_ruangan: Number(asset.id_ruangan), id_user: asset.id_user || null, serial_number: asset.serial_number?.trim() || null, purchase_year: asset.purchase_year || null, price: asset.price || null, stock: Number(asset.stock), specifications: asset.specifications ? { detail: asset.specifications } : null }, () => { setAsset(blankAsset); setAssetEditingId(null) })}>
                    ...
                    {needsTechnicalSpecs && (
                        <div style={{ gridColumn: '1 / -1', marginTop: '10px' }}><TechnicalSpecsForm categoryName={selectedCategory?.name} values={technicalSpecs} onChange={(event) => setTechnicalSpecs((current) => ({ ...current, [event.target.name]: event.target.value }))} /></div>
                    )}
    ...
    {showPartModal && (
        <div className="modal-backdrop" onClick={() => setShowPartModal(false)}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: '#fff', borderRadius: '12px', padding: '24px', width: '90%', maxWidth: '800px', maxHeight: '90vh', overflowY: 'auto' }}>
                ...
                <form className="inventory-form" onSubmit={(event) => submit(event, partEditingId ? `/inventory/spareparts/${partEditingId}` : '/inventory/spareparts', partEditingId ? 'PUT' : 'POST', { ...part, id_category: part.id_category || null, stock: Number(part.stock), min_stock: Number(part.min_stock) }, () => { setPart(blankPart); setPartEditingId(null) })}>
                    <div style={{ gridColumn: '1 / -1', background: '#e0f2fe', padding: '16px', borderRadius: '8px', border: '1px solid #bae6fd' }}>
...
function WorkSection({ title, items, columns, labels, itemsPerPage = 10, children, renderActions }) {
  ...
  return (
    <section className="inventory-section" style={{ marginTop: '20px' }}>
      <div className="inventory-section-title"><div><h3>{title}</h3><span>{items.length} data tercatat</span></div></div>
      {children}
