        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
             ...
             {isAdmin && (
                <button 
                  ...
                  style={{ backgroundColor: '#059669', color: 'white', padding: '10px 16px', borderRadius: '8px', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}
                >
                  + Tambah Aset
                </button>
             )}
...
function WorkSection({ title, items, columns, labels, children }) { return <section className="inventory-section" style={{ marginTop: '20px' }}><SectionTitle title={title} count={items.length} search="" setSearch={() => {}} />{children}<div style={{ overflowX: 'auto', background: '#fff', border: '1px solid #e2e8f0', borderRadius: '10px' }}><table className="inventory-table" style={{ width: '100%', borderCollapse: 'collapse' }}><thead><tr style={{ textAlign: 'left', background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>{labels.map((label) => <th key={label} style={{ padding: '14px 16px', color: '#475569', fontSize: '0.85rem' }}>{label}</th>)}</tr></thead><tbody>{items.map((item) => <tr key={item.id || `${item.asset_code}-${item.movement_date}`} style={{ borderBottom: '1px solid #f1f5f9' }}>{columns.map((column) => <td key={column} style={{ padding: '12px 16px', fontSize: '0.85rem' }}>{item[column] ?? '-'}</td>)}</tr>)}</tbody></table></div></section> }
