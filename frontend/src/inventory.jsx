...
function Inventory({ token, user, onBack, onError }) {
  ...
  return <section className="inventory-page">
      ...
      <section className="inventory-section">
        ...
        {isAdmin && (
          <form className="inventory-form" onSubmit={(event) => submit(event, '/inventory/spareparts', 'POST', { ...part, id_category: part.id_category || null }, () => setPart(blankPart))}>
            ...
            <div style={{ gridColumn: '1 / -1' }}><FormActions label="Tambah sparepart" /></div>
          </form>
        )}
          ...
          <tbody>
            {paginatedParts.map((item) => (
              <tr key={item.id}>
                ...
                <td className={item.low_stock ? 'low-stock' : ''}>{item.stock} {item.low_stock && '(!)'}</td>
                <td>{item.unit || '-'}</td>
                <td>{item.supplier || '-'}</td>
                {isAdmin && <td><button className="danger-button" onClick={() => remove(`/inventory/spareparts/${item.id}`)}>Hapus</button></td>}
              </tr>
            ))}
          </tbody>
        </table>
    ...
    {tab === 'movements' && (
      <WorkSection title="Pergerakan Aset" items={data.movements} itemsPerPage={ITEMS_PER_PAGE} columns={['asset_code', 'movement_type', 'from_room', 'to_room', 'user_name', 'movement_date']} labels={['Aset', 'Jenis', 'Asal', 'Tujuan', 'Pengguna', 'Tanggal']}>
        <form className="inventory-form" onSubmit={(event) => submit(event, '/inventory/movements', 'POST', { ...movement, id_asset: Number(movement.id_asset), id_user: movement.id_user || null, from_location: movement.from_location || null, to_location: movement.to_location || null }, () => setMovement(blankMovement))}>
          <Field label="Aset *"><Select name="id_asset" value={movement.id_asset} onChange={update(setMovement)} required><option value="">Pilih aset</option>{assetOptions}</Select></Field>
          ...
          <Field label="Kondisi"><Input name="condition" value={movement.condition} onChange={update(setMovement)} /></Field>
    ...
    {tab === 'transactions' && (
      <WorkSection title="Transaksi Sparepart" items={data.transactions} itemsPerPage={ITEMS_PER_PAGE} columns={['sparepart_name', 'transaction_type', 'quantity', 'transaction_date', 'ticket_title', 'pic_name']} labels={['Sparepart', 'Jenis', 'Jumlah', 'Tanggal', 'Referensi tiket', 'PIC']}>
        <form className="inventory-form" onSubmit={(event) => submit(event, '/inventory/transactions', 'POST', { ...transaction, id_sparepart: Number(transaction.id_sparepart), quantity: Number(transaction.quantity), id_tiket: transaction.id_tiket || null }, () => setTransaction(blankTransaction))}>
    ...
    {tab === 'master-products' && (
      <section className="inventory-section">
        ...
        {isAdmin && (
          <form className="inventory-form" onSubmit={(event) => { const { sku_code, product_name, id_category, default_price, ...specifications } = masterProduct; submit(event, '/inventory/master-products', 'POST', { sku_code, product_name, id_category: id_category || null, default_price: default_price || null, specifications: Object.fromEntries(Object.entries(specifications).filter(([, value]) => String(value).trim())) }, () => setMasterProduct(blankMasterProduct)) }}>
            ...
            <div style={{ gridColumn: '1 / -1' }}><FormActions label="Simpan master produk" /></div>
          </form>
        )}
        <table className="inventory-table">
          <thead>
          ...
          <tbody>
            {paginatedMaster.map((item) => (
              <tr key={item.id}>
                ...
                {isAdmin && <td><button className="danger-button" type="button" onClick={() => remove(`/inventory/master-products/${item.id}`)}>Hapus</button></td>}
              </tr>
            ))}
  ...
  return (
    <section className="inventory-section" style={{ marginTop: '20px' }}>
      <div className="inventory-section-title">
        <div>
          <h3>{title}</h3>
          <span>{items.length} data tercatat</span>
        </div>
      </div>
      {children}
      <table className="inventory-table">
...
function AssetTable({ items, isAdmin, remove, onEdit, onView }) { 
  return (
    <table className="inventory-table">
      <thead>
        <tr>
...
function AssetDetailModal({ asset, onClose }) { 
  ...
  const fields = [['Kode Aset', asset.asset_code], ['Kategori', asset.category_name], ['Lokasi', asset.ruangan], ['Pengguna', asset.user_name], ['Merek/Model', asset.brand_model], ['Serial Number', asset.serial_number], ['Tanggal Pembelian', asset.purchase_year], ['Harga', asset.price ? `Rp ${Number(asset.price).toLocaleString('id-ID')}` : '-'], ['Status', asset.status], ['Kondisi', asset.condition], ['Catatan', asset.notes]]; 
  return (
