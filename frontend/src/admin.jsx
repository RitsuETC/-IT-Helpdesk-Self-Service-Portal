...
function Admin({ token, articles, onChanged, onError, user }) {
  ...
  const removeSetup = async (path, label) => {
    if (!(await confirmAction(`Hapus ${label}? Data yang masih digunakan tidak dapat dihapus.`))) return
  ...
  <section className="admin-articles user-list"><h3>Kelola Role Pengguna</h3><input className="user-search" value={userQuery} onChange={(e) => setUserQuery(e.target.value)} placeholder="Cari nama atau email" aria-label="Cari pengguna" />
