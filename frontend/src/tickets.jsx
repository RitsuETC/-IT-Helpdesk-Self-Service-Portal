...
function TicketDetail({ token, user, ticket, onBack, onError }) {
  ...
  return <section className="ticket-detail"><header className="detail-header"><button onClick={onBack} aria-label="Kembali">‹</button><h2>Detail Tiket</h2></header><h3>{ticket.judul}</h3>
    <dl><dt>ID Tiket</dt><dd>HD-{ticket.id}</dd>{(isAdmin || user.role === 'teknisi') && <><dt>Pelapor</dt><dd>{ticket.pelapor_nama || '-'}</dd></>}<dt>Ruangan</dt><dd>{ticket.nama_ruangan}</dd><dt>Masalah</dt><dd>{ticket.deskripsi}</dd><dt>Prioritas</dt><dd><span className={`priority-dot ${priorityClass(ticket.prioritas)}`} /></dd><dt>Status</dt><dd>{status}</dd><dt>Dibuat</dt><dd>{fmt(ticket.created_at)}</dd><dt>Diselesaikan</dt><dd>{fmt(ticket.resolved_at)}</dd><dt>kategori</dt><dd>{ticket.nama_kategori || '-'}</dd></dl>
