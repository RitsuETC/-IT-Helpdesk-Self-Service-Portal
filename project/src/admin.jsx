...
function Admin({ token, articles, onChanged, onError }) {
  ...
  return <section className="admin-page"><h2>Panel Admin</h2><div className="admin-layout">
    ...
    <form className="admin-form" onSubmit={(e) => addSetup(e, '/admin/categories', { nama_kategori: categoryName }, () => setCategoryName(''))}><h3>Kategori Tiket</h3><label>Nama kategori<input value={categoryName} onChange={(e) => setCategoryName(e.target.value)} required /></label><button>Tambah Kategori</button><ul>{setup.categories.map((item) => <li key={item.id}>{item.nama_kategori}</li>)}</ul></form>
    <form className="admin-form" onSubmit={(e) => addSetup(e, '/admin/rooms', { ruangan: roomName }, () => setRoomName(''))}><h3>Ruangan</h3><label>Nama ruangan<input value={roomName} onChange={(e) => setRoomName(e.target.value)} required /></label><button>Tambah Ruangan</button><ul>{setup.rooms.map((item) => <li key={item.id}>{item.ruangan}</li>)}</ul></form>
    ...
    {['admin', 'teknisi', 'user'].map((role) => <section className="role-group" key={role}><h4>{role}</h4>{visibleUsers.filter((user) => user.role === role).map((user) => <article key={user.id}><div><b>{user.nama}</b><small>{user.email}</small></div><select value={user.role} onChange={(e) => changeRole(user.id, e.target.value)}><option value="user">User</option><option value="admin">Admin</option><option value="teknisi">Teknisi</option></select></article>)}{!visibleUsers.some((user) => user.role === role) && <p className="empty-role">Belum ada akun.</p>}</section>)}
  </section>
  </section>
}
